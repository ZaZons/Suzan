...
module.exports = {
	data: new SlashCommandBuilder()
		.setName('log-role')
		.setDescription('Selects a channel as your role log, role related logs will be sent there')
	...
	async execute(interaction) {
		try {
			...
			const sql = `UPDATE guilds SET roleLog = '${logChannel.id}' WHERE guilds.id = '${interaction.guild.id}'`;
			connection.query(sql).catch(console.log);

			const embed = new MessageEmbed()
				.setColor(process.env.color)
				.setTitle('Role log updated')
			...
			await logChannel.send('Hi, this is your new role log');
